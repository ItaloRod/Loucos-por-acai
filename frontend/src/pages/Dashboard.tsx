...
export const Dashboard = () => {
  ...
  const stats = [
    { label: 'Vendas Hoje', value: 'R$ 840,00', change: '+12% vs ontem', icon: <TrendingUp size={22} />, color: 'bg-green-500/10 text-green-600 dark:text-green-500' },
    { label: 'Pedidos Pendentes', value: '5', change: '3 delivery, 2 retirada', icon: <ClipboardList size={22} />, color: 'bg-yellow-500/10 text-yellow-600 dark:text-yellow-500' },
    { label: 'Clientes Ativos', value: '142', change: '+8 cadastrados essa semana', icon: <Users size={22} />, color: 'bg-blue-500/10 text-blue-600 dark:text-blue-500' },
    { label: 'Alertas de Estoque', value: '2', change: 'Leite Condensado, Granola', icon: <PackageAlert size={22} />, color: 'bg-red-500/10 text-red-600 dark:text-red-500' },
  ];
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <section className="bg-card border border-border p-6 rounded-2xl flex items-center justify-between flex-wrap gap-4 shadow-sm">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Bem-vindo ao painel operacional do Loucos por Açaí. Nível de acesso: <strong className="text-primary font-bold">{user?.role}</strong>.
          </p>
        </div>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, idx) => (
          <div key={idx} className="bg-card border border-border p-6 rounded-2xl shadow-sm space-y-4 hover:shadow-md transition-all duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <section className="lg:col-span-1 bg-card border border-border p-6 rounded-2xl shadow-sm space-y-4 flex flex-col justify-between">
          <div className="space-y-4">
            <h3 className="font-bold text-base text-foreground border-b border-border/50 pb-2">Ações Rápidas</h3>
        ...
        <section className="lg:col-span-2 bg-card border border-border p-6 rounded-2xl shadow-sm space-y-4">
          <h3 className="font-bold text-base text-foreground border-b border-border/50 pb-2">Últimos Pedidos Recebidos</h3>
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3.5 rounded-xl bg-muted/10 border border-border/40">
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-muted-foreground">Cliente: Paulo Rodrigues • Delivery</p>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-[10px] font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-full uppercase">Preparando</span>
            ...
            <div className="flex items-center justify-between p-3.5 rounded-xl bg-muted/10 border border-border/40">
              ...
              <div className="flex items-center gap-3">
                <span className="text-[10px] font-bold text-yellow-600 bg-yellow-500/10 px-2 py-0.5 rounded-full uppercase">Pendente</span>
            ...
            <div className="flex items-center justify-between p-3.5 rounded-xl bg-muted/10 border border-border/40">
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-muted-foreground">Cliente: Maria Souza • PDV Caixa</p>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-[10px] font-bold text-green-600 bg-green-500/10 px-2 py-0.5 rounded-full uppercase">Concluído</span>
