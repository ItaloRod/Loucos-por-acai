...
| `cpf` | `String(14)` | `str` | Unique, Not Null, Index | Cadastro de Pessoa Física (ID Fiscal brasileiro). |
| `phone` | `String(20)` | `str` | - | Telefone de contato. |
...
| `deleted_at` | `DateTime(timezone=True)` | `datetime` | Nullable | Controle de *Soft Delete*. |
...
### StampTransaction
Histórico de ganhos e resgates de selos fidelidade.
...
| `type` | `Enum('EARNED', 'REDEEMED')`| `enum` | Not Null | Tipo de operação (Ganho ou Resgate). |
...
| `status` | `Enum(...)` | `enum` | Not Null | Ver sessão de *Status do Pedido* abaixo. |
...
| `day_of_week` | `Integer` | `int` | Not Null | 0 = Segunda, 6 = Domingo. |
...
2. O ORM irá possuir um *Custom Base/Query Class* ou métodos explícitos para filtrar resultados válidos: `query.filter(Product.deleted_at.is_(None))`.
...
Um script de *seeding* será desenvolvido (ex: `scripts/seed.py`) gerando o estado inicial do sistema no ambiente. 
...
3. **Validação de Data Integrity**: Após o *Load*, um script de conferência vai testar os quantitativos das ordens antigas versus faturamento exportado do Django para garantir que não houveram perdas (ex: batimento de subtotal e total).
